...
async function renderVideo(id,language,token){
  ...
  try{
    ...
    function setBuffering(show){
      buffering.classList.toggle('show',!!show);
      buffering.setAttribute('aria-hidden',show?'false':'true');
      if(show) status.textContent='Buffering 480p…';
    }
    ...
    function force480(){
      ...
      try{
        ...
        const has480=levels.includes('large');
        player.setPlaybackQuality('large');
        const actual=String(player.getPlaybackQuality?.()||'');
        const locked=actual==='large' || has480===false;
        setBuffering(!locked && player.getPlayerState?.()===YT.PlayerState.PLAYING);
        return locked;
      }catch{return false;}
    }
    ...
    function enforce480(){
      ...
      try{
        ...
        const quality=String(player.getPlaybackQuality?.()||'');
        if(st===YT.PlayerState.PLAYING && quality && quality!=='large'){
          setBuffering(true);
          player.setPlaybackQuality('large');
        }else if(st===YT.PlayerState.PLAYING && quality==='large'){
          setBuffering(false);
        }
      }catch{}
    }
