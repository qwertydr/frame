...
async function renderVideo(id,language,token){
  ...
  try{
    ...
    async function syncTime(data){
      const duration=Number(data?.duration)||await player.getDuration().catch(()=>0);
      const seconds=Number(data?.seconds)||0;
      current.textContent=fmtTime(seconds);
      remaining.textContent=`Remaining: ${fmtTime(Math.max(0,duration-seconds))}`;
      progress.max=duration||100;
      progress.value=Math.min(seconds,duration||seconds);
    }
    ...
    volume.addEventListener("input",async()=>{
      try{
        ...
        if(value>0){ await player.setMuted(false); mute.textContent="🔊"; }
        else { await player.setMuted(true); mute.textContent="🔇"; }
      }catch{}
    });

    progress.addEventListener("input",async()=>{
      try{ await player.setCurrentTime(Number(progress.value)); }catch{}
    });
    ...
    full.addEventListener("click",async()=>{
      ...
      catch{ document.getElementById("customPlayer").requestFullscreen?.(); }
    });

    player.on("loaded",async()=>{ await syncTime({seconds:0}); });
    ...
    player.on("ended",async()=>{
      ...
      await syncTime({seconds:await player.getDuration().catch(()=>0)});
    });

    player.on("error",()=>{ status.textContent="Video error"; });
    try{ await player.setVolume(1); }catch{}
  }catch(error){
