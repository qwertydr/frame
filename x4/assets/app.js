...
async function renderVideo(id,language,token){
  ...
  /*
   * Player-only configuration.
   * captionsEnabled: true = request YouTube CC when available.
   * subtitleUrl: optional WebVTT URL for the custom top-positioned captions.
   * force480Viewport: keeps the player engine at a 854x480 reference canvas;
   * the CSS scales that canvas responsively on smaller screens.
   */
  const playerConfig={
    force480Viewport:true,
    captionsEnabled:lecture.captionsEnabled!==false,
    captionLanguage:String(lecture.captionLanguage||'en'),
    captionFontSize:Number.isFinite(Number(lecture.captionFontSize))
      ? Number(lecture.captionFontSize) : 0,
    subtitleUrl:String(lecture.subtitleUrl||'').trim()
  };
  ...
  shell(`
    <div class="topbar">
      ...
      <button class="btn secondary" id="backBtn" type="button">← Curriculum</button>
    </div>
    ...
    <section class="hero video-card">
      ...
      <div class="player" id="customPlayer">
        ...
        <div class="player-caption" id="playerCaption" aria-live="polite" aria-hidden="true"></div>
  ...
  try{
    ...
    const captionEl=document.getElementById('playerCaption');

    let player=null;
    let customCues=[];
    let captionsEnabled=playerConfig.captionsEnabled;
    let duration=0;
    let isSeeking=false;
    let lastSeconds=0;
    let progressTimer=0;
    ...
    function parseVTT(text){
      ...
      const toSeconds=(v)=>{
        ...
        if(parts.length===3) return Number(parts[0])*3600+Number(parts[1])*60+Number(parts[2].replace(',','.'));
        if(parts.length===2) return Number(parts[0])*60+Number(parts[1].replace(',','.'));
        return Number(parts[0].replace(',','.'))||0;
      };
      for(const block of blocks){
        ...
        const start=toSeconds(times[0].replace(/<[^>]+>/g,''));
        const end=toSeconds(times[1].replace(/<[^>]+>/g,'').split(' ')[0]);
        const textLines=lines.slice(timeIndex+1).join('\\n')
          .replace(/<v[^>]*>/gi,'').replace(/<\/v>/gi,'')
          .replace(/<[^>]+>/g,'');
        if(end>start && textLines) cues.push({start,end,text:textLines});
      }
      return cues;
    }

    async function loadCustomCaptions(){
      if(!captionEl) return;
      captionEl.textContent='';
      captionEl.setAttribute('aria-hidden','true');
      if(!playerConfig.subtitleUrl) return;
      try{
        const response=await fetch(playerConfig.subtitleUrl,{cache:'no-store'});
        if(!response.ok) throw new Error('Subtitle request failed');
        customCues=parseVTT(await response.text());
      }catch(error){
        customCues=[];
        console.warn('Custom subtitle unavailable:',error);
      }
    }

    function syncCustomCaption(currentTime){
      if(!captionEl || !customCues.length) return;
      const cue=customCues.find(x=>currentTime>=x.start && currentTime<x.end);
      captionEl.textContent=cue?cue.text:'';
      captionEl.setAttribute('aria-hidden',cue?'false':'true');
    }
    ...
    function createPlayer(){
      player=new YT.Player('youtubePlayer',{
        ...
        playerVars:{
          ...
          cc_lang_pref:playerConfig.captionLanguage
        },
        events:{
          onReady(){
            ...
            if(captionsEnabled){
              try{
                if(player.getOptions?.('captions')?.length){
                  player.setOption('captions','fontSize',playerConfig.captionFontSize);
                }
              }catch{}
            }
            loadCustomCaptions();
            ...
            stateTimer=window.setInterval(refreshState,500);
          },
          onStateChange(event){
            if(event.data===YT.PlayerState.PLAYING) setPlayingUI(true);
            else if(event.data===YT.PlayerState.PAUSED) setPlayingUI(false);
            else if(event.data===YT.PlayerState.ENDED) setEndedUI();
            refreshState();
          },
          onApiChange(){
          ...
          onPlaybackQualityChange(){refreshState();},
          onPlaybackRateChange(){refreshState();},
          onError(){status.textContent='Video error';}
        }
      });
    }
