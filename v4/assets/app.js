...
ROOT.innerHTML=`<div class="login-wrap"><div class="login-card"><div class="brand">IARCO 2026</div><p class="muted">Academic Research Bootcamp Portal</p><form id="loginForm"><div class="field"><label>Email</label><input id="loginEmail" type="email" autocomplete="username" required></div><div class="field"><label>Password</label><input id="loginPassword" type="password" autocomplete="current-password" required></div>${message?`<div class="error">${esc(message)}</div>`:""}<button class="btn" style="width:100%;margin-top:16px">Login</button></form><p class="muted small">IARCO 2026 Sponsored by <a class="sponsor-link" href="https://www.savemyexams.com/" target="_blank" rel="noopener noreferrer">SaveMyExams</a> &amp; <a class="sponsor-link" href="https://domain.me/" target="_blank" rel="noopener noreferrer">Domain.Me</a></p></div></div>`;
document.getElementById("loginForm").addEventListener("submit",async e=>{e.preventDefault();const b=e.currentTarget.querySelector("button"),email=document.getElementById("loginEmail").value.trim().toLowerCase(),password=document.getElementById("loginPassword").value;b.disabled=true;b.innerHTML='<span class="button-spinner"></span> Loading…';try{await prefetchPortalData();const u=state.users.find(x=>String(x.email).toLowerCase()===email&&x.password===password);if(!u){renderLogin("Invalid email or password.");return;}state.user={email:u.email,name:u.name,institution:u.institution,languages:Array.isArray(u.languages)?u.languages:[u.language].filter(Boolean)};localStorage.setItem(SESSION_KEY,JSON.stringify(state.user));location.hash="";showLoader("Preparing your dashboard…");setTimeout(()=>{renderDashboard();showQuickIntroIfNeeded();},350);}catch(err){console.error(err);renderLogin("Unable to load portal data. Please try again.");}});}
...
function showQuickIntroIfNeeded(){const key=`iarco_intro_seen_v4_${state.user.email}`;if(localStorage.getItem(key)==="1")return;const o=document.createElement("div");o.className="intro-overlay";o.innerHTML=`<div class="intro-card"><div class="intro-progress"><span id="introStepLabel">1 / 3</span></div><div class="intro-step active" data-step="1"><div class="intro-icon">👋</div><h2>Welcome to your IARCO 2026 Portal</h2><p class="muted">This quick introduction shows you how to use your bootcamp portal.</p></div><div class="intro-step" data-step="2"><div class="intro-icon">📚</div><h2>Use Next to continue</h2><p class="muted">Click <b>Next</b> on the dashboard to open your assigned curriculum and languages.</p></div><div class="intro-step" data-step="3"><div class="intro-icon">⏱️</div><h2>Watch your timeline</h2><p class="muted">The sidebar and dashboard contain live assignment countdowns, Submit links and Rules.</p></div><div class="intro-actions"><button class="btn secondary" id="introSkip">Skip</button><button class="btn" id="introNext">Next</button></div></div>`;document.body.appendChild(o);let step=1;const render=()=>{o.querySelectorAll(".intro-step").forEach(x=>x.classList.toggle("active",Number(x.dataset.step)===step));o.querySelector("#introStepLabel").textContent=`${step} / 3`;o.querySelector("#introNext").textContent=step===3?"Get Started":"Next"};const close=()=>{localStorage.setItem(key,"1");o.remove()};o.querySelector("#introSkip").onclick=close;o.querySelector("#introNext").onclick=()=>{if(step===3)close();else{step++;render()}}}
function renderDashboard(){
  shell(`
    ...
    <section class="hero">
      <h1>Welcome to the IARCO 2026 Academic Research Bootcamp</h1><p class="sponsor-line">IARCO 2026 Sponsored by <a href="https://www.savemyexams.com/" target="_blank" rel="noopener noreferrer">SaveMyExams</a> &amp; <a href="https://domain.me/" target="_blank" rel="noopener noreferrer">Domain.Me</a></p><p class="muted">
  ...
  shell(`
    ...
    <section class="hero video-card">
      ...
      <div class="player" id="customPlayer">
        ...
        <div class="player-brand-wrap" aria-hidden="true"><img class="player-brand" src="assets/logo-watermark.svg" alt="" draggable="false" onerror="this.style.display='none';this.parentElement.classList.add('logo-fallback')"><span class="player-brand-fallback">IARCO 2026</span></div>
...
async function boot(){state.user=getSession();if(!state.user){renderLogin();return;}showLoader("Loading your portal…");try{await prefetchPortalData();const match=location.hash.match(/^#video\/([^/]+)$/);if(match){let route=null;try{route=JSON.parse(sessionStorage.getItem(ROUTE_KEY)||"null")}catch{}if(route&&route.t===match[1])await renderVideo(route.moduleId,route.language,match[1]);else notFound()}else renderDashboard();updateCountdowns()}catch(e){console.error(e);ROOT.innerHTML=`<div class="error-page"><main><h1>Portal configuration error</h1><p>${esc(e.message)}</p></main></div>`}}
/*
