...
async function renderVideo(id,language,token){
      ...
      <div class="player" id="customPlayer">
        ...
        <iframe
          ...
          src="https://player.vimeo.com/video/${encodeURIComponent(
            lecture.videoId||module.videoId||""
  ...
  try{
    ...
    const player=new Vimeo.Player(
      document.getElementById("vimeoFrame"),
      {
        controls:false,
        title:false,
        byline:false,
    ...
    function setPlayingUI(playing){
      ...
      cover.classList.toggle("hidden",playing);
      status.textContent=playing?"Playing":"Paused";
    }

    async function refreshState(){
      try{
        const [paused,muted,vol,rate]=await Promise.all([
          player.getPaused(),
          player.getMuted(),
          player.getVolume(),
        ...
        volume.value=String(Math.max(0,Math.min(1,Number(vol)||0)));
        speed.value=String(rate||1);
      }catch{}
    }
    ...
    async function toggle(){
      try{
        ...
        if(paused) await player.play(); else await player.pause();
    ...
    const commitSeek=async()=>{
      const value=Number(progress.value)||0;
      try{ await player.setCurrentTime(value); }catch{}
      isSeeking=false;
      renderTime(value);
    };
    ...
    speed.addEventListener("change",async()=>{
      try{await player.setPlaybackRate(Number(speed.value));}catch{}
    });

    full.addEventListener("click",async()=>{
      try{await player.requestFullscreen();}
      catch{playerEl.requestFullscreen?.();}
    });

    player.on("loaded",async(data)=>{
      duration=Number(data?.duration)||await player.getDuration().catch(()=>0);
      progress.max=duration||100;
      renderTime(0);
      await refreshState();
    });
    ...
    player.on("timeupdate",data=>{
      if(Number(data?.duration)>0) duration=Number(data.duration);
      renderTime(Number(data?.seconds)||0);
    });

    player.on("play",()=>setPlayingUI(true));
    player.on("playing",()=>setPlayingUI(true));
    ...
    player.on("volumechange",refreshState);
    player.on("playbackratechange",refreshState);
