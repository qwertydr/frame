...
function noticeHTML(){const n=String(cfg().notice||'').trim();return !n||sessionStorage.getItem('notice_closed_v7')?'':`<div class="site-notice" id="siteNotice" role="status" aria-label="Portal notice"><span>${esc(n)}</span><button id="closeNotice" type="button" aria-label="Close notice">×</button></div>`;}
...
function bindChrome(){
  ...
  close?.addEventListener('click',()=>{
    sessionStorage.setItem('notice_closed_v7','1');
...
function loadYouTubeAPI(){
  return new Promise((resolve,reject)=>{
    ...
    s.async=true;
    s.onerror=()=>reject(new Error("Unable to load YouTube IFrame API."));
...
async function renderVideo(id,language,token){
  ...
  try{
    route=JSON.parse(
      sessionStorage.getItem(ROUTE_KEY)||"null"
    );
  }catch{
    route=null;
  }
  ...
  const lectureIndex=Math.max(
    0,
    Math.min(
      Number(route.lectureIndex)||0,
      lectures.length-1
    )
  );
    ...
    <section class="hero video-card">
      ...
      <div class="video-heading">
        ...
        <div class="lecture-title-small">
          Lecture ${lectureIndex+1} of ${lectures.length}
          — ${esc(lecture.title||module.title)}
        </div>
      </div>
      ...
      <div class="player" id="customPlayer">
        ...
        <div
          id="youtubeFrame"
          class="youtube-frame"
          data-video-id="${esc(lecture.videoId||module.videoId||"")}"
          aria-label="Course video"
        ></div>

        <div class="player-shield" aria-hidden="true"></div>

        <div class="player-thumbnail" id="playerThumbnail" aria-hidden="true">
          <img
            id="playerThumbnailImage"
            src="https://i.ytimg.com/vi/${encodeURIComponent(lecture.videoId||module.videoId||"")}/hqdefault.jpg"
            alt=""
            draggable="false"
          >
        </div>
        ...
        <div class="player-controls" aria-label="Video controls">
          ...
          <span class="player-time" id="remainingTime">Remaining: 0:00</span>
          ...
          <span class="label">${esc(cfg().brand||"IARCO")}</span>
        </div>
      </div>

      <div class="lecture-navigation">

        <div class="lecture-navigation-title">
          <small>
            ${esc(lecture.title||module.title)}
          </small>
        </div>
        ...
        lecture.resource
          ? `
            <div class="lecture-resource">
              ${resourceButton(lecture.resource)}
            </div>
          `
          : ""
      }
  ...
  if(hasPrevious){
    document.getElementById("prevLecture").addEventListener(
      "click",
      ()=>{
        openVideo(
          module.id,
  ...
  if(hasNext){
    document.getElementById("nextLecture").addEventListener(
      "click",
      ()=>{
        openVideo(
          module.id,
  ...
  try{
    ...
    const videoId=String(lecture.videoId||module.videoId||"").trim();
    const iframeHost=document.getElementById("youtubeFrame");
    const playerEl=document.getElementById("customPlayer");

    if(!videoId || !iframeHost || !playerEl) throw new Error("YouTube video is not configured.");
    ...
    const thumbnailImage=document.getElementById("playerThumbnailImage");
    ...
    function renderTime(seconds){
      ...
      remaining.textContent=`Remaining: ${fmtTime(Math.max(0,duration-safe))}`;
      if(!isSeeking){
    ...
    function setPlayingUI(playing){
      ...
      big.textContent=ended?"↻":(playing?"❚❚":"▶");
      big.setAttribute("aria-label",ended?"Replay lesson":(playing?"Pause lesson":"Play lesson"));
      playerEl.classList.toggle("is-playing",playing);
      cover.classList.toggle("hidden",playing || ended);
      status.textContent=ended?"Completed":(playing?"Playing":"Paused");
    }
    ...
    function syncTime(){
      ...
      try{
        ...
        if(state===YT.PlayerState.PLAYING){
          renderTime(player.getCurrentTime());
    ...
    function refreshState(){
      ...
      try{
        ...
        volume.value=String(Math.max(0,Math.min(1,(Number(vol)||0)/100)));
        const rateValue=String(rate||1);
        if([...speed.options].some(o=>o.value===rateValue)) speed.value=rateValue;
      }catch{}
    }
    ...
    function setVolume(value){
      ...
      mute.textContent=v===0?"🔇":"🔊";
    }

    function enterCustomFullscreen(){
      const request=playerEl.requestFullscreen||playerEl.webkitRequestFullscreen||playerEl.msRequestFullscreen;
      if(request){
        Promise.resolve(request.call(playerEl)).catch(()=>{});
      }
    }
    ...
    function updateFullscreenButton(){
      fullscreenActive=!!(document.fullscreenElement===playerEl || document.webkitFullscreenElement===playerEl);
      full.textContent=fullscreenActive?"⛶":"⛶";
      full.setAttribute("aria-label",fullscreenActive?"Exit fullscreen":"Fullscreen");
      playerEl.classList.toggle("is-fullscreen",fullscreenActive);
    }

    if(thumbnailImage){
      thumbnailImage.addEventListener("error",()=>{
        if(!thumbnailImage.src.endsWith("/default.jpg")){
          thumbnailImage.src=`https://i.ytimg.com/vi/${encodeURIComponent(videoId)}/default.jpg`;
        }
      });
    }
    ...
    progress.addEventListener("input",()=>{
      ...
      remaining.textContent=`Remaining: ${fmtTime(Math.max(0,duration-value))}`;
    });
    const commitSeek=()=>{
      if(!playerReady) return;
      const value=Number(progress.value)||0;
    ...
    full.addEventListener("click",()=>{
      if(document.fullscreenElement===playerEl || document.webkitFullscreenElement===playerEl) exitCustomFullscreen();
    ...
    function syncDuration(){
      ...
      try{
        ...
        if(d>0){
          duration=d;
          progress.max=d;
          renderTime(lastSeconds);
        }
      }catch{}
    }

    function syncAvailableRates(){
      if(!playerReady || !speed) return;
      try{
        const available=player.getAvailablePlaybackRates();
        if(Array.isArray(available) && available.length){
          [...speed.options].forEach(option=>{
            option.disabled=!available.some(rate=>String(rate)===option.value);
          });
          const currentRate=String(player.getPlaybackRate()||1);
          if(!available.some(rate=>String(rate)===currentRate)) speed.value=String(available[0]);
        }
      }catch{}
    }
    ...
    const player=new YT.Player("youtubeFrame",{
      ...
      events:{
        ...
        onStateChange:(event)=>{
          switch(event.data){
            ...
            case YT.PlayerState.PAUSED:
              stopTicker();
              setPlayingUI(false);
              renderTime(event.target.getCurrentTime());
              break;
            case YT.PlayerState.ENDED:
              stopTicker();
              duration=Number(event.target.getDuration())||duration;
              renderTime(duration);
              ended=true;
              setPlayingUI(false);
              thumbnail.classList.add("visible");
              cover.classList.add("hidden");
              big.textContent="↻";
              big.setAttribute("aria-label","Replay lesson");
        ...
        onPlaybackRateChange:(event)=>{
          const value=String(event.data||1);
